...
const MAPPING: Partial<Record<keyof Metrics, string>> = {
  monthlyTreasuryBalance: 'treasury_balance_dot',
} as const;
export class ParityPolkadotTreasuryNetworkDatasource extends AbstractNetworkDataSource<Config> {
  private static data: Array<Record<string, string>> = parse(
    fs.readFileSync(__dirname + '/treasury.csv', {
      encoding: 'utf-8',
    }),
  );
  ...
  async getItems({ subcategories }: Params<typeof networkMetrics>): Promise<{
    ...
    const networks =
      ParityPolkadotTreasuryNetworkDatasource.getSubcategories(subcategories);

    const items: Entity<Metrics, Presets>[] = [];
    for (const network of networks) {
      const pieces = ParityPolkadotTreasuryNetworkDatasource.data
        .filter(({ chain_name }) => chain_name === network)
  ...
  static getSubcategories(subcategories: SubcategoryId[]) {
    return subcategories.filter((chain) =>
      ParityPolkadotTreasuryNetworkDatasource.data.find(
        (piece) => piece.chain_name === chain,
      ),
    );
  }
