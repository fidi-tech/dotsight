...
const MAPPING: Partial<Record<keyof Metrics, string>> = {
  monthlyAmountStacked: 'total_dot_staked',
} as const;
export class ParityAmountStackedNetworkDatasource extends AbstractNetworkDataSource<Config> {
  private static data: Array<Record<string, string>> = parse(
    fs.readFileSync(__dirname + '/amount-stacked.csv', {
      encoding: 'utf-8',
    }),
  );
  ...
  async getItems({ subcategories }: Params<typeof networkMetrics>): Promise<{
    ...
    const networks =
      ParityAmountStackedNetworkDatasource.getSubcategories(subcategories);

    const items: Entity<Metrics, Presets>[] = [];
    for (const network of networks) {
      const pieces = ParityAmountStackedNetworkDatasource.data
  ...
  static getSubcategories(subcategories: SubcategoryId[]) {
    return subcategories.filter((chain) =>
      ParityAmountStackedNetworkDatasource.data.find(
