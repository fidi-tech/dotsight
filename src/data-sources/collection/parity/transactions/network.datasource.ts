...
const MAPPING: Partial<Record<keyof Metrics, string>> = {
  monthlyTransactionCount: 'number_of_transactions',
} as const;
export class ParityTransactionsNetworkDatasource extends AbstractNetworkDataSource<Config> {
  private static data: Array<Record<string, string>> = parse(
    fs.readFileSync(__dirname + '/monthly-transactions-by-parachain.csv', {
      encoding: 'utf-8',
    }),
  );
  ...
  async getItems({ subcategories }: Params<typeof networkMetrics>): Promise<{
    ...
    const networks =
      ParityTransactionsNetworkDatasource.getSubcategories(subcategories);

    const items: Entity<Metrics, Presets>[] = [];
    for (const network of networks) {
      const pieces = ParityTransactionsNetworkDatasource.data
  ...
  static getSubcategories(subcategories: SubcategoryId[]) {
    return subcategories.filter((chain) =>
      ParityTransactionsNetworkDatasource.data.find(
